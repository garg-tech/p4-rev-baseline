...
def run_ssc_cmd(switch, cmd, debug=True):
    ...
    print("port cmd- ", switch_port, cmd)
...
def load_topology(topo_file_path):
    ...
    json_hosts = j['hosts']
    # we added
    json_switches = j['switches']
    # json_switches = list(j['switches'].keys())
    json_links = run_exercise.parse_links(j['links'])
    mn_topo = run_exercise.ExerciseTopo(json_hosts, json_switches, json_links, "logs", "simple_switch_grpc", "/home/p4/tutorials/exercises/Hula-hoop/pcaps")
