...
class ExerciseTopo(Topo):
    ...
    def __init__(self, hosts, switches, links, log_dir, bmv2_exe, pcap_dir, **opts):
        ...
        print("Host Links -> ", host_links)
        print("Switch Links -> ", switch_links)
        ...
        print("Sorted Host Links -> ", host_links)
        print("Sorted Switch Links -> ", switch_links)
        #we added
        print("other stuff ", bmv2_exe, pcap_dir)
        for sw, params in list(switches.items()):
            # print("params: ", params["program"])
        ...
        for link in switch_links:
            self.addLink(link['node1'], link['node2'],
                        delay=link['latency'], bw=link['bandwidth'])
            p1 = self.addSwitchPort(link['node1'], link['node2'])
            p2 = self.addSwitchPort(link['node2'], link['node1'])
            # print(p1, " - ", link['node1'], " - ", link['node2'])
    ...
    def printPortMapping(self):
        ...
        for sw in sorted(self.sw_port_mapping.keys()):
            print ("%s: " % sw,)
            if sw not in self.key_mapping:
                self.key_mapping[sw] = []
            for portno, node2 in self.sw_port_mapping[sw]:
                print ("%d:%s\t" % (portno, node2),)
                if 's' in node2:
                    self.key_mapping[sw].append((portno, node2[1:]))
            print()

    def portMappingFile(self):
        with open("keys.json", "w") as out:
            json.dump(self.key_mapping, out)
...
class ExerciseRunner:
    ...
    def create_network(self):
        ...
        # print("current json cls- ", self.switch_json)
